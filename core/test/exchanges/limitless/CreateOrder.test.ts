...
import { LimitlessAuth } from '../../../src/exchanges/limitless/auth';

// Mock the dependencies
jest.mock('../../../src/exchanges/limitless/auth');

describe('LimitlessExchange - createOrder', () => {
    ...
    beforeEach(() => {
        ...
        // Mock the constructor of LimitlessAuth to return our instance
        (LimitlessAuth as jest.Mock).mockImplementation(() => mockAuthInstance);

        exchange = new LimitlessExchange({ privateKey: '0x1234567890' });
    });

    describe('Unit Tests', () => {
        it('should throw error if not authenticated', async () => {
            const noAuthExchange = new LimitlessExchange();
    ...
    describe('Integration Tests', () => {
        ...
        if (!shouldRun) {
            console.log('Skipping Limitless createOrder integration tests: No private key or test outcome ID found');
        }

        let realExchange: LimitlessExchange;

        beforeAll(() => {
            if (shouldRun) {
                // Use real implementation, not mocks
                jest.unmock('../../../src/exchanges/limitless/auth');
                const { LimitlessExchange } = require('../../../src/exchanges/limitless');
                realExchange = new LimitlessExchange({ privateKey: process.env.POLYMARKET_PRIVATE_KEY });
            }
        });
        ...
        // Limitless uses proxy, so no gas, but be careful.
