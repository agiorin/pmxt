...
curl supports WebSocket when `curl --version` lists `ws` and `wss` under protocols. These piped examples require curl `8.16.0` or newer so curl can send the subscription frame from stdin.
...
curl --version
# Confirm the Protocols line includes ws and wss.
