...
while True:
    ...
    print(f"  bid={event.orderbook.bids[0].price} ask={event.orderbook.asks[0].price}")
```
...
while (true) {
  ...
  console.log(`  bid=${event.orderbook.bids[0].price} ask=${event.orderbook.asks[0].price}`);
}
```
